...
class CanvasMobileBet extends Component<IProps, IState> {
    ...
    updateUi() {
        if (isMobile) {
            UIManager.getRef("bet_text") && UIManager.setText("bet_text", this.props.coinList[this.props.selectedCoin] / this.divider);
            let balance;
    ...
    setCoinValue() {
        if (this.props.jurisdictionKey == "social") {
                UIManager.getRef("Bet_button_Coin_Value") && (UIManager.getRef("Bet_button_Coin_Value").visible = false);
                UIManager.getRef("Bet_button_Bet") && (UIManager.getRef("Bet_button_Bet").visible = true);
        }
        else {
                UIManager.getRef("Bet_button_Coin_Value") && (UIManager.getRef("Bet_button_Coin_Value").visible = true);
                UIManager.getRef("Bet_button_Bet") && (UIManager.getRef("Bet_button_Bet").visible = false);
        }
    }
    ...
    shouldComponentUpdate(nextProps: Readonly<IProps>, nextState: Readonly<IState>, nextContext: any): boolean {
        ...
        if (nextProps.selectedCoin !== this.props.selectedCoin) {
            UIManager.getRef("bet_text") && UIManager.setText("bet_text", nextProps.coinList[nextProps.selectedCoin] / this.divider);
        }
    ...
    setButtonState(balance: number) {
        if (this.props.selectedCoin == 0) {
            ...
            UIManager.getRef("countMinus") && (UIManager.getRef("countMinus").texture = Texture.from("minus_p.png"));
        }


        if (this.props.selectedCoin + 1 == this.props.coinList.length || this.props.betList[this.props.selectedCoin + 1] > balance) {
            UIManager.getRef("countPlus") && (UIManager.getRef("countPlus").texture = Texture.from("plus_disable_p.png"));
            UIManager.getRef("countMinus") && (UIManager.getRef("countMinus").texture = Texture.from("minus_p.png"));
        }
        else {
            UIManager.getRef("countPlus").texture = Texture.from("plus_p.png");
    ...
    maxButtonState() {
        UIManager.getRef("maxbetBtn") && (UIManager.getRef("maxbetBtn").texture = Texture.from("max.png"));
    }
    handleEvent = (e: any) => {
        let balance;
        this.props.transitionBalance == 0 ? balance = this.props.balance : balance = this.props.transitionBalance;
